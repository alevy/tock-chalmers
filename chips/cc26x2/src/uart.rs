...
impl UART {
    ...
    pub fn handle_interrupt(&self) {
        ...
        self.transaction.buffer.take().map(|buffer| {
            self.transaction.index.set(self.transaction.index.get() + 1);
            if self.transaction.index.get() < self.transaction.length.get() {
                self.send_byte(buffer[self.transaction.index.get()]);
                self.transaction.buffer.put(Some(buffer));
            } else {
...
impl kernel::hil::uart::UART for UART {
    ...
    fn init(&self, params: kernel::hil::uart::UARTParams) {
        self.initialize();
        // Stop bits, parity and HW flow control currently ignored params
    ...
    fn transmit(&self, tx_data: &'static mut [u8], tx_len: usize) {
        ...
        self.transaction.buffer.put(Some(tx_data));
        self.transaction.length.set(tx_len);
        self.transaction.index.set(0);
    }
